...
import { HomeDesignPage, type HomeDesignPageProps } from "./HomeDesignPage";

/*
 * 참고: 이 프로젝트의 Vitest는 Storybook addon + Playwright 브라우저 모드로만
 * 구성되어 있어(jsdom/testing-library 미사용), .test.tsx에서는 DOM 렌더링을 하지
 * 않는다. 실제 렌더링/입력 흐름 검증은 HomeDesignPage.stories.tsx 의 play function
 * 에서 수행한다. 여기서는 컴포넌트 계약(props 위임)을 단위로 검증한다.
 */

describe("HomeDesignPage", () => {
  it("HomeDesignPage 컴포넌트는 함수다", () => {
    expect(typeof HomeDesignPage).toBe("function");
  });

  it("onAuthClick prop을 받는다", () => {
    const handler = vi.fn();
    const props: HomeDesignPageProps = { onAuthClick: handler };
    expect(props.onAuthClick).toBe(handler);
  });

  it("onWriteClick prop을 받는다", () => {
    const handler = vi.fn();
    const props: HomeDesignPageProps = { onWriteClick: handler };
    expect(props.onWriteClick).toBe(handler);
  });

  it("activeSort / onSortChange prop을 받는다", () => {
    const handler = vi.fn();
    const props: HomeDesignPageProps = {
      activeSort: "popular",
  ...
  it("onPostClick prop을 받는다 (게시글 상세보기 이동용)", () => {
    const handler = vi.fn();
    const props: HomeDesignPageProps = { onPostClick: handler };
